    ...
    glCompressedTexImage2D: function(target, level, internalFormat, width, height, border, imageSize, data) {
        ...
        if (GameGlobal._webgpuASTCEnabled && GameGlobal._webgpuASTCDecoder && isAstcFmt) {
          ...
          if (astcBytes) {
            ...
            // WebGL2: sRGB ASTC 用 SRGB8_ALPHA8 + RGBA 上传
            var _phInternalFormat = _isSrgbAstc ? 0x8C43 : GLctx.RGBA;
            ...
            GameGlobal._webgpuASTCDecoder.decodeAndInjectToGLTexture(lastTid, astcBytes, width, height, blk[0], blk[1], {
              ...
              if (GameGlobal.logmanager) {
                GameGlobal.logmanager.warn("[WebGPU ASTC] decodeAndInject failed:", e && e.message)
              }
            })
          }
          return
    ...
    glCompressedTexSubImage2D: function(target, level, xoffset, yoffset, width, height, format, imageSize, data) {
        ...
        if (GameGlobal._webgpuASTCEnabled && GameGlobal._webgpuASTCDecoder && isAstcFmt) {
          ...
          if (astcBytes) {
            ...
            // WebGL2: sRGB ASTC 用 SRGB8_ALPHA8 + RGBA 上传
            var _phInternalFormat2 = _isSrgbAstcSub ? 0x8C43 : GLctx.RGBA;
            var _phFormat2 = GLctx.RGBA;
            if (_texObj2) {
              GLctx.bindTexture(_bindTarget2, _texObj2);
              GLctx.texImage2D(_imgTarget2, level, _phInternalFormat2, width, height, 0,
                               _phFormat2, GLctx.UNSIGNED_BYTE, _zeros2);
