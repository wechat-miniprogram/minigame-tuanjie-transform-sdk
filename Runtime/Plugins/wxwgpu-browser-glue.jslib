// ★ Resolved copy generated by resolve_wxwgpu_glue.py — do not edit by hand ★
// C_STRUCTS offsets (301 refs / 65 types) baked from the SDK webgpu.h for wasm32,
// `PTHREADS` renamed to `USE_PTHREADS`, so this links on emsdk 3.1.x whose bundled webgpu.h
// lacks/mis-lays-out these structs. Source: sdk/js/wxwgpu-browser-glue.js

// ★ 自动生成文件 — 请勿手动编辑 ★
// 由 tools/gen_glue.py 从 emsdk 4.0.13 的 libwebgpu.js 变换生成。
// 生成日期：2026-08-21
//
// wxwgpu 浏览器后端 glue（自包含，两端安全加载）：
...
//
// 升级 emsdk 时重新生成：python3 tools/gen_glue.py <libwebgpu.js 路径>
// 注意：本文件的 struct 布局（C_STRUCTS）在 demo 链接期由 emcc 展开，
// 必须与 adapter include/webgpu/webgpu.h 保持 ABI 一致。
...
var LibraryWebGPU = {
  ...
  $WebGPU: {
    errorCallback: (callback, type, message, userdata) => {
      var sp = stackSave();
      var messagePtr = allocateUTF8OnStack(message);
  ...
  wxwgpu_browser_wgpuDevicePopErrorScope__deps: ['$callUserCallback'],
  ...
  wxwgpu_browser_wgpuDeviceSetUncapturedErrorCallback__deps: ['$callUserCallback'],
  ...
  wxwgpu_browser_wgpuDeviceCreateBindGroup__deps: ['$readI53FromI64'],
  wxwgpu_browser_wgpuDeviceCreateBindGroup: function(deviceId, descriptor) {
    ...
    function makeEntry(entryPtr) {
      ...
      if (bufferId) {
        var size = {{{ `(${makeGetValue('entryPtr', (24) + 4, 'i32')} * 4294967296 + ${makeGetValue('entryPtr', 24, 'i32', false, true)})` }}};
  ...
  wxwgpu_browser_wgpuDeviceCreateComputePipelineAsync__deps: ['$callUserCallback', '$generateComputePipelineDesc'],
  wxwgpu_browser_wgpuDeviceCreateComputePipelineAsync: function(deviceId, descriptor, callback, userdata) {
    ...
    device.createComputePipelineAsync(desc).then((pipeline) => {
      ...
      callUserCallback(() => {
        var sp = stackSave();
        var messagePtr = allocateUTF8OnStack(pipelineError.message);
  ...
  wxwgpu_browser_wgpuDeviceCreateRenderPipelineAsync__deps: ['$callUserCallback', '$generateRenderPipelineDesc'],
...
#endif
    var queue = WebGPU.mgrQueue.get(queueId);
    var cmds = Array.from({{{ makeHEAPView(`32`, 'commands', `commands + commandCount * 4`)}}},
      (id) => WebGPU.mgrCommandBuffer.get(id));
    queue.submit(cmds);
  },

  wxwgpu_browser_wgpuQueueOnSubmittedWorkDone__deps: ['$callUserCallback'],
  ...
  wxwgpu_browser_wgpuQueueWriteTexture: function(queueId,
  ...
  wxwgpu_browser_wgpuCommandEncoderResolveQuerySet: function(encoderId, querySetId, firstQuery, queryCount,
  ...
  wxwgpu_browser_wgpuShaderModuleGetCompilationInfo__deps: ['$callUserCallback', 'malloc', 'free'],
  ...
  wxwgpu_browser_wgpuBufferGetConstMappedRange__deps: ['memalign', 'free'],
  ...
  // In webgpu.h offset and size are passed in as size_t.
  // And library_webgpu assumes that size_t is always 32bit in emscripten.
  wxwgpu_browser_wgpuBufferGetMappedRange__deps: ['$zeroMemory', 'memalign', 'free'],
  ...
  // In webgpu.h offset and size are passed in as size_t.
  // And library_webgpu assumes that size_t is always 32bit in emscripten.
  wxwgpu_browser_wgpuBufferMapAsync__deps: ['$callUserCallback'],
  ...
  wxwgpu_browser_wgpuBufferGetSize: function(bufferId) {
    var buffer = WebGPU.mgrBuffer.get(bufferId);
    // 64-bit
    var r64 = buffer.size; setTempRet0((r64 / 0x100000000) >>> 0); return r64 >>> 0;
  },
  ...
  wxwgpu_browser_wgpuRenderPassEncoderExecuteBundles: function(passId, count, bundlesPtr) {
    ...
    var bundles = Array.from({{{ makeHEAPView(`32`, 'bundlesPtr', `bundlesPtr + count * 4`) }}},
  ...
  wxwgpu_browser_wgpuInstanceCreateSurface__deps: ['$findCanvasEventTarget'],
  ...
  wxwgpu_browser_wgpuInstanceRequestAdapter__deps: ['$callUserCallback'],
    ...
    navigator["gpu"]["requestAdapter"](opts).then((adapter) => {
      ...
      callUserCallback(() => {
        if (adapter) {
          ...
          var sp = stackSave();
          var messagePtr = allocateUTF8OnStack('WebGPU not available on this system (requestAdapter returned null)');
          {{{ makeDynCall('viiii', 'callback') }}}({{{ gpu.RequestAdapterStatus.Unavailable }}}, 0, messagePtr, userdata);
          stackRestore(sp);
        }
      });
      ...
      callUserCallback(() => {
        var sp = stackSave();
        var messagePtr = allocateUTF8OnStack(ex.message);
  ...
  wxwgpu_browser_wgpuAdapterGetInfo__deps: [],
  wxwgpu_browser_wgpuAdapterGetInfo: function(adapterId, info) {
    var adapter = WebGPU.mgrAdapter.get(adapterId);
    {{{ gpu.makeCheckDescriptor('info') }}}

    var vendorPtr = allocateUTF8(adapter.info.vendor);
    {{{ makeSetValue('info', 4, 'vendorPtr', '*') }}};
    var architecturePtr = allocateUTF8(adapter.info.architecture);
    {{{ makeSetValue('info', 8, 'architecturePtr', '*') }}};
    var devicePtr = allocateUTF8(adapter.info.device);
    {{{ makeSetValue('info', 12, 'devicePtr', '*') }}};
    var descriptionPtr = allocateUTF8(adapter.info.description);
  ...
  wxwgpu_browser_wgpuAdapterGetProperties__deps: [],
  ...
  wxwgpu_browser_wgpuAdapterRequestDevice__deps: ['$callUserCallback'],
    ...
    if (descriptor) {
      ...
      if (requiredLimitsPtr) {
        ...
        function setLimitU32IfDefined(name, limitOffset) {
          ...
          if (value != {{{ gpu.LIMIT_U32_UNDEFINED }}} && (name in adapter.limits)) {
        ...
        function setLimitU64IfDefined(name, limitOffset) {
          ...
          if ((limitPart1 != 0xFFFFFFFF || limitPart2 != 0xFFFFFFFF) && (name in adapter.limits)) {
...
var LibraryWxwgpuBrowserExtra = {
  // webgpu.cpp: wgpuCreateInstance 返回固定句柄 1（glue 内 assert instanceId===1）
