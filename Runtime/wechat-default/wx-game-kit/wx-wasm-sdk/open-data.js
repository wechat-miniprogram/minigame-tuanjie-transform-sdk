...
function getOpenDataContext(mode) {
  ...
  if (!isSupportSharedCanvasMode) {
    if (mode === 'ScreenCanvas') {
      console.warn('[wx-wasm-sdk]: 当前环境不支持 ScreenCanvas 模式');
    }
  ...
  console.log(`[wx-wasm-sdk]: 当前开放数据域为 ${sharedCanvasMode} 模式`);
  // @ts-ignore
...
function hookUnityRender() {
  ...
  const { GL } = WXGameKit.Module;
  const Module = WXGameKit.Module;
  const gl = GL.currentContext.GLctx;
  const isLinearColorSpace = WXGameKit.config.runtime.colorSpace === 'Linear';
...
function stopHookUnityRender() {
  ...
  if (textureObject) {
    ...
    if (typeof textureId !== 'undefined' && unityTextureObject) {
...
export default {
  ...
  WXShowOpenData(id, x, y, width, height) {
    ...
    if (width <= 0 || height <= 0) {
      console.error('[wx-wasm-sdk]: WXShowOpenData要求 width 和 height 参数必须大于0');
    }

    if (!cachedOpenDataContext) {
      console.warn('[wx-wasm-sdk]: 请先调用 WXGetOpenDataContext');
    }
