...
function WX_TCPSocketWriteBuffer(id, dataPtr, dataLength) {
    ...
    obj.write(WXGameKit.loader.module.HEAPU8.buffer.slice(dataPtr, dataPtr + dataLength));
}
...
function WX_TCPSocketOnMessage(id, needInfo) {
    ...
    const callback = (res) => {
        ...
        if (needInfo) {
            ...
            WXGameKit.loader.module.dynCall_viiiii(wxTCPSocketOnMessageCallback, idPtr, messagePtr, res.message.length || res.message.byteLength, localInfoPtr, remoteInfoPtr);
            WXGameKit.loader.module._free(localInfoPtr);
            WXGameKit.loader.module._free(remoteInfoPtr);
        }
        else {
            WXGameKit.loader.module.dynCall_viiiii(wxTCPSocketOnMessageCallback, idPtr, messagePtr, res.message.length || res.message.byteLength, 0, 0);
        }
        WXGameKit.loader.module._free(idPtr);
        WXGameKit.loader.module._free(messagePtr);
    };
