...
function jsAudioCreateUncompressedSoundClip(buffer, error, length) {
  const soundClip = {
    ...
    getData(ptr, length) {
      ...
      const output = GameGlobal.WXGameKit.gameInstance.Module.HEAPF32.subarray(startOutputBuffer, startOutputBuffer + (length >> 2));
...
function jsAudioCreateCompressedSoundClip(audioData, ptr, length) {
  ...
  if (isSupportBufferURL) {
    ...
    if (WXGameKit.fs.getCachePath(tempFilePath)) {
...
function jsAudioCreateUncompressedSoundClipFromPCM(channels, length, sampleRate, ptr) {
  if (WEBAudio.audioContext) {
    ...
    for (let i = 0; i < channels; i++) {
      ...
      copyToChannel.apply(buffer, [GameGlobal.WXGameKit.gameInstance.Module.HEAPF32.subarray(offs, offs + length), i, 0]);
    }
  ...
  playUrl(startTime, url, startOffset, volume, soundClip) {
    ...
    try {
      ...
      this.source.mediaElement.onStop(() => {
        ...
        if (this.callback) {
          GameGlobal.WXGameKit.gameInstance.Module.dynCall_vi(this.callback, [this.userData]);
        }
      });
...
export default {
  ...
  _JS_Sound_Load(ptr, length, decompress) {
    ...
    const audioData = heapBufferSlice(GameGlobal.WXGameKit.gameInstance.Module.HEAPU8, ptr, length);
