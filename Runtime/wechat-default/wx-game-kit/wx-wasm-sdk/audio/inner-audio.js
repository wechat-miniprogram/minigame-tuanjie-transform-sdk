...
const funs = {
  ...
  getFullUrl(v) {
    if (!/^https?:\/\//.test(v) && !/^wxfile:\/\//.test(v)) {
      ...
      const dataFileCDN = (WXGameKit && WXGameKit.config && WXGameKit.config.network && WXGameKit.config.network.dataFileCDN) || '';
      const assetPrefix = (WXGameKit && WXGameKit.config && WXGameKit.config.fs && WXGameKit.config.fs.assetPrefix) || 'Assets';
      const cdnPath = `${dataFileCDN.replace(/\/$/, '')}/${assetPrefix.replace(/\/$/, '')}`;
    ...
    return Promise.all(list.map((v) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (!downloadingAudioMap[src]) {
          ...
          if (funs.checkLocalFile(src)) {
          ...
          else if (!WXGameKit.fs.getIsAssetCacheable(src)) {
            ...
            xmlhttp.onsave = () => {
              localAudioMap[src] = WXGameKit.fs.getCachePath(src);
              funs.handleDownloadEnd(src, true);
            };
            xmlhttp.onsavefail = () => {
              funs.handleDownloadEnd(src, false);
  ...
  checkLocalFile(src) {
    ...
    const path = WXGameKit.fs.getCachePath(src);
    if (path) {
