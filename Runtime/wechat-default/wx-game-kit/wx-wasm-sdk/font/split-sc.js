...
function parseTableToDataView(fontDataView, tableName, startOffset = 0) {
    ...
    WXGameKit.Logger.pluginError(`\tTable#${tableName} not found in DataView`);
    return undefined;
}
...
export default function splitTTCToBufferOnlySC(arrayBuffer) {
    ...
    if (toBytesInt32(tag) !== 'ttcf') {
        WXGameKit.Logger.pluginError('input not a valid ttc file');
        return undefined;
    }
    ...
    if (!fontSCHeaderOffset) {
        WXGameKit.Logger.pluginError('SC Font not found in TTC File.');
        return undefined;
    }
