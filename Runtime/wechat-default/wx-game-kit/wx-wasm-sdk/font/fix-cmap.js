export default function fixCmapTable(arrayBuffer) {
    ...
    for (let i = 0; i < tableCount; i++) {
        ...
        if (tag === 0x636D6170) { 
            ...
            WXGameKit.Logger.pluginLog(`[font]cmapCheckSubOffset [${cmapCheckSumOffset}], cmapCheckSum [${cmapCheckSum}], cmapOffset [${cmapOffset}], cmapLength [${cmapLength}]`);
        }
    }
    if (cmapOffset === 0) {
        WXGameKit.Logger.pluginError('[font]not found cmap');
        return false;
    }
    ...
    for (let i = 0; i < numTables; i++) {
        ...
        if (platformId === 0 && encodingId === 5) {
            if (i === (numTables - 1)) { 
                ...
                WXGameKit.Logger.pluginLog(`[font]targetSubtableOffset ${targetSubtableOffset}`);
            }
            break;
        }
        subtableOffset += 8;
    ...
    WXGameKit.Logger.pluginLog('[font]not found cmap subtable');
