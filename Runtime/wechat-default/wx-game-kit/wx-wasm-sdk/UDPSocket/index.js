...
function allocateUTF8WithBuffer(str, buffers, id) {
  const Module = WXGameKit.loader.module;
...
function allocFromUdpPool(id, dataPtr, dataLength) {
  ...
  // 必须拷贝：wx.send 异步，发送期间 wasm 可能改写 HEAPU8
  pool.u8.set(WXGameKit.loader.module.HEAPU8.subarray(dataPtr, dataPtr + dataLength), start);
  ...
  const callback = (res) => {
    try {
      ...
      Module.dynCall_viiiiiiii(
        wxUDPSocketOnMessageCallbackV2,
        idPtr, msgPtr, msgLen,
        family,
        remoteAddrPtr, remotePort,
        localAddrPtr, localPort,
      );
