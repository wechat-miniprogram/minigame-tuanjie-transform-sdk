...
function allocateAndSet(byteArray) {
  const ptr = WXGameKit.loader.module._malloc(byteArray.length);
  WXGameKit.loader.module.HEAPU8.set(byteArray, ptr);
  return ptr;
}
...
function convertStringToPointer(str) {
  const byteArray = WXGameKit.loader.module.lengthBytesUTF8(str) + 1;
  const ptr = WXGameKit.loader.module._malloc(byteArray);
  WXGameKit.loader.module.stringToUTF8(str, ptr, byteArray);
  return ptr;
}
...
function stringToUint8ArrayWithLength(str) {
  ...
  const strBytesLength = WXGameKit.loader.module.lengthBytesUTF8(str);
  const strBytes = new Uint8Array(WXGameKit.loader.module.HEAPU8.buffer, strPtr, strBytesLength);
  ...
  WXGameKit.loader.module._free(strPtr);
  return result;
}
