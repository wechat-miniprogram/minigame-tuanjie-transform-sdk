...
export default {
  WX_OneWayFunction(functionName, successType, failType, completeType, conf, callbackId) {
    ...
    if (lowerFunctionName === 'login') {
      ...
      if (WXGameKit.status && WXGameKit.status.setGameStage) {
        WXGameKit.status.setGameStage(config.sceneId);
      }
    }
  ...
  WX_ClassOnEventFunction(className, functionName, returnType, id, eventName) {
    ...
    // WXPageManager 同样使用 (eventName, callback) 的两参回调形式，老版由“2025.9.27”排查发现并修复
  ...
  WX_RequestSubscribeSystemMessage(conf, callbackId) {
    conf = formatJsonStr(conf);
    if (conf.msgTypeList && typeof conf.msgTypeList === 'string') {
      try {
        conf.msgTypeList = JSON.parse(conf.msgTypeList);
      } catch (e) {
        console.error('Failed to parse msgTypeList:', e);
      }
    }
  ...
  WX_ReportScene(option) {
    wx.reportScene(formatJsonStr(option));
  },
  ...
  WX_CreateVideoDecoder() {
    ...
    this.VideoDecoderList = this.VideoDecoderList || {};
    const list = this.VideoDecoderList;
    const key = uid();
    list[key] = obj;
    return key;
  },
  ...
  WX_UpdateManagerOnCheckForUpdate(id) {
    const obj = this.UpdateManagerList[id];
    obj.UpdateManagerOnCheckForUpdateList = obj.UpdateManagerOnCheckForUpdateList || [];
    const callback = (res) => {
    ...
    obj.UpdateManagerOnCheckForUpdateList.push(callback);
    obj.onCheckForUpdate(callback);
  },
  WX_UpdateManagerOnUpdateFailed(id) {
    const obj = this.UpdateManagerList[id];
    obj.UpdateManagerOnUpdateFailedList = obj.UpdateManagerOnUpdateFailedList || [];
    const callback = (res) => {
      formatResponse('GeneralCallbackResult', res);
      const resStr = JSON.stringify({
        callbackId: id,
    ...
    obj.UpdateManagerOnUpdateFailedList.push(callback);
    obj.onUpdateFailed(callback);
  },
  WX_UpdateManagerOnUpdateReady(id) {
    const obj = this.UpdateManagerList[id];
    obj.UpdateManagerOnUpdateReadyList = obj.UpdateManagerOnUpdateReadyList || [];
    const callback = (res) => {
      formatResponse('GeneralCallbackResult', res);
      const resStr = JSON.stringify({
        callbackId: id,
    ...
    obj.UpdateManagerOnUpdateReadyList.push(callback);
    obj.onUpdateReady(callback);
  },
  WX_VideoDecoderGetFrameData(id) {
    const obj = this.VideoDecoderList[id];
    if (obj) {
      return JSON.stringify(formatResult('FrameDataOptions', obj.getFrameData(), id));
    }
    return JSON.stringify(formatResult('FrameDataOptions'));
  },
  ...
  WX_VideoDecoderOff(id, eventName) {
    const obj = this.VideoDecoderList[id];
    obj.VideoDecoderOffList = obj.VideoDecoderOffList || [];
    obj.VideoDecoderOffList.forEach((v) => {
      obj.off(eventName, v);
    });
    obj.VideoDecoderOffList = [];
  },
  WX_VideoDecoderOn(id, eventName) {
    const obj = this.VideoDecoderList[id];
    obj.VideoDecoderOnList = obj.VideoDecoderOnList || [];
    const callback = (res) => {
      const resStr = JSON.stringify({
        callbackId: id + eventName,
        res: JSON.stringify(res),
      });
      moduleHelper.send('_VideoDecoderOnCallback', resStr);
    };
    obj.VideoDecoderOnList.push(callback);
