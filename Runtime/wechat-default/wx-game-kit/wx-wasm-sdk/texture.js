...
const getCdnBase = () => ((WXGameKit && WXGameKit.config && WXGameKit.config.network && WXGameKit.config.network.dataFileCDN) || '').replace(/\/$/, '');
const getAssetPrefix = () => ((WXGameKit && WXGameKit.config && WXGameKit.config.fs && WXGameKit.config.fs.assetPrefix) || 'Assets').replace(/\/$/, '');
...
const pngPath = WXGameKit.config.runtime.colorSpace && WXGameKit.config.runtime.colorSpace === 'Linear' ? 'lpng' : 'png';
...
const mod = {
  getSupportedExtensions() {
    ...
    const isWebGL2 = WXGameKit.config.runtime.contextConfig.contextType === 2;
  ...
  requestFile(path, width, height, format, limitType) {
    const cid = path;
    const url = `${getCdnBase()}/${getAssetPrefix()}/Textures/${format}/${width}/${path}.txt`;
    const xmlhttp = new WXGameKit.network.XMLHttpRequest();
  ...
  downloadFile(path, width, height) {
    const url = `${getCdnBase()}/${getAssetPrefix()}/Textures/${pngPath}/${width}/${path}.png`;
    const cid = path;
    const cache = WXGameKit.fs.getCachePath(url);
    if (cache) {
      mod.callbackPngFile(cache, cid);
    }
    else {
      if (WXGameKit.fs.getIsTextureCacheable(path)) {
        const xmlhttp = new WXGameKit.network.XMLHttpRequest();
        xmlhttp.responseType = 'arraybuffer';
        xmlhttp.open('GET', url, true);
...
GameGlobal.ParalleLDownloadTexture = function (filename) {
  // 空值保护：dataFileCDN 缺失时用空串占位（replace(''，'') 是无操作，等价于跳过 CDN 前缀剥离）
  filename = filename.replace((WXGameKit && WXGameKit.config && WXGameKit.config.network && WXGameKit.config.network.dataFileCDN) || '', '').replace(/^\//, '');
  filename = `/${filename}`;
  if (WXGameKit.gameInstance.textureBundles[filename]) {
    WXGameKit.gameInstance.textureBundles[filename].forEach((v) => {
      const f = GameGlobal.TextureCompressedFormat;
      if (!f) {
        const p = `${getCdnBase()}/${getAssetPrefix()}/Textures/png/${v.w}/${v.p}.png`;
        const image = wx.createImage();
        image.crossOrigin = '';
        image.src = p;
      }
      else if (f !== 'pvr') {
        const http = new WXGameKit.network.XMLHttpRequest();
        const p = `${getCdnBase()}/${getAssetPrefix()}/Textures/${f}/${v.w}/${v.p}.txt`;
...
canvasContext.addCreatedListener(() => {
  if (WXGameKit.config.runtime.usedTextureCompression) {
    mod.getSupportedExtensions();
