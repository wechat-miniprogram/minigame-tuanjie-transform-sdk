...
export const { platform, system } = wx.getDeviceInfo ? wx.getDeviceInfo() : wx.getSystemInfoSync();
...
// iOS系统版本>=16.0才允许进入游戏（低版本可能触发卡死）
const isIOSMinSystemVersionValid = compareVersion(systemVersion, '16.0');
...
// 是否用了webgl2
const isWebgl2 = () => WXGameKit.config.runtime.contextType === 2;
// 是否支持BufferURL
...
// iOS系统版本低于16.0则禁止进入游戏
const isIOSMinSystemVersionInvalid = isIOS && !isIOSMinSystemVersionValid;
...
export const isNeedMainLoopMinDelay = isAndroid && !compareVersion(version, '8.0.70');
/**
 * 判断环境是否可使用coverview
...
export default () => {
  if (!isDevtools) {
    if (isPcInvalid
      ...
      || isBrotliInvalid
      || isIOSMinSystemVersionInvalid) {
      let updateWechat = true;
      let content = '当前微信版本过低\n请更新微信后进行游戏';
      if (isIOS) {
        // iOS系统版本过低优先提示升级系统，避免误导用户去升级微信
        if (isIOSMinSystemVersionInvalid) {
          content = '当前iOS系统版本过低\n请升级至iOS 16.0及以上版本后进行游戏';
          updateWechat = false;
        }
        else if (!isIOSH5SystemVersionValid || (isWebgl2SystemVersionInvalid() && isIOS)) {
          content = '当前操作系统版本过低\n请更新iOS系统后进行游戏';
          updateWechat = false;
        }
      }
