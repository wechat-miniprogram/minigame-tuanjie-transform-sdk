...
namespace WeChatWASM
{
    ...
    public class WXPCHPInitScript : MonoBehaviour
    {
        ...
        // SendMsgSync 已删除(Mojo 同步调用死锁)。

        // 查询 Mojo IPC 通道是否就绪(host_remote 已 bind)。
        // 新版 pchp_sdk 真等 browser 侧 ConnectPchp bind 完成才返回 true。
        // 优雅降级:dll 未导出时走 5 次重试 fallback。
        [DllImport(DLL_NAME, CallingConvention = CallingConvention.Cdecl)]
        [return: MarshalAs(UnmanagedType.U1)]
        private static extern bool IsChannelReady();

        // ─── Windows 消息泵（SendAppEventSync while 阻塞期间代泵,防 Windows 5s 未响应判定） ───
        // PM_REMOVE = 1
        ...
        // ─── 自定义链路字段 ───
        // 提供 SendAppEvent / SendAppEventSync / HandleHostEvent 三个方法,与 AHP MinaSDKAHP.cs 签名对齐。
    ...
    /// <summary>SendAppEventSync 伪同步回包</summary>
    ...
    /// <summary>SendAppEventSync 请求发出时刻(UTC),用于往返耗时计算</summary>
    ...
    /// <summary>首次 SendAppEventSync 是否已完成(成功或重试成功后置 true)</summary>
    ...
    private bool? _channelReadyAvailable = null;
        ...
        public void Initialize()
        {
            ...
            try
            {
                ...
                // ★ GetDeviceInfo 预热:Initialize 完成后立即异步发出,回包到达时缓存到 _cachedDeviceInfo,
                //   后续业务调用命中缓存,绕开 SendAppEventSyncInternal 同步阻塞。
        ...
        /// 与 AHP 平台 sendAppEvent(eventName, jsonStr) 双参数语义对齐。
        ///
        /// ⚠️ 不走 SendMsgSync 真同步(Mojo 同步调用死锁)。
        /// </summary>
        /// <param name="eventName">事件名称</param>
        ...
        public string SendAppEventSync(string eventName, string jsonStr)
        {
            ...
            // ★ GetDeviceInfo 缓存命中优先返回(绕开 SendAppEventSyncInternal 同步阻塞)
            ...
            // 优先路径:dll 暴露 IsChannelReady 时,等 host_remote bind 完成 + 单次拿回包
            //   WaitForChannelReady(6000) 等 bind 完成
            //   SendAppEventSyncInternal(2000) 拿回包(bind 完成后回包必毫秒级,2s 是兜底)
            if (IsChannelReadyAvailable())
            {
                if (WaitForChannelReady(6000))
                {
                    return SendAppEventSyncInternal(eventName, jsonStr, 2000);
                }
                Debug.LogWarning($"[WXPCHPInitScript] SendAppEventSync 等待通道就绪超时(6s): {eventName}");
                return "";
            }

            // Fallback:dll 未暴露 IsChannelReady 时走多次重试
            //   首次:5 × 2s = 10s 覆盖 bind 延迟窗口
            //   后续:单次 5s(bind 已完成)
            string result;
            if (!_firstSyncCallDone)
            {
                ...
                {
                    result = SendAppEventSyncInternal(eventName, jsonStr, 2000);
                    ...
                    if (attempt < 5)
                    {
                        Debug.LogWarning($"[WXPCHPInitScript] SendAppEventSync 第 {attempt} 次超时(2s), 通道可能未就绪, 重试: {eventName}");
                    }
                    else
                    {
            ...
            {
                // 后续调用：host_remote 已绑定，单次 5s 足够
                result = SendAppEventSyncInternal(eventName, jsonStr, 5000);
            }
...
#if UNITY_STANDALONE_WIN
                // ★ Windows 消息泵:阻塞期间持续 PeekMessage/TranslateMessage/DispatchMessage
                //   让消息队列被泵着,防 Windows 5s 判"未响应"+白屏。
                //   风险:DispatchMessage 可能触发窗口事件重入,PCHP 阶段窗口事件少,重入风险低。
        ...
        /// <summary>
        /// GetDeviceInfo 预热:Initialize 完成后立即异步发出(fire-and-forget)。
        /// 回包到达 _messageQueue 时被 ProcessIncomingMessage 识别并缓存到 _cachedDeviceInfo,
        /// 后续业务调用直接命中缓存,绕开 SendAppEventSyncInternal 同步阻塞。
        ...
        // 注:原 BuildLocalDeviceInfo/BuildSystemString 已移除(单一接口兜底治标不治本,问题本质在 Mojo bind 延迟,跟具体 API 无关)。
        //   保留通用方案:Windows 消息泵 + GetDeviceInfo 预热缓存 + IsChannelReady 真等 bind。
        ...
        /// 阻塞主线程,每 100ms 让出(Thread.Sleep(100))。
        /// </summary>
        ...
        private void ProcessIncomingMessage(string messageJson)
        {
            ...
            else if (jsonData.ContainsKey("eventName"))
            {
...

                {
                    ...
                    {
                        // ★ 预热回包:requestId 匹配 _preheatRequestId 时只缓存,不触发当前调用解锁
                        ...
                        if (_syncCallActive)
                        {
                            ...
                            // ★ 当前调用是 GetDeviceInfo 时,同步缓存真实数据
